...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
   ...
   for(int i = 0; i < s.size(); i++){
       ...
       int loc = i;
       while(loc > 0 && comp(s[i - 1], nextStudent)){
           s[loc] = s[loc - 1];
           loc--;
       }
       s[loc] = nextStudent;
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    vector<Student*> sps; //vector of pointers to students
    //set each pointer to corresponding student in auxStudents
    for(int i = 0; i < auxStudents.size(); i++){
        sps.push_back(&auxStudents[i]);
    }
   
    //sort pointer of vectors with STL algorithm (comp parameter as order)
    sort(sps.begin(), sps.end(), comp);

    //replace students with students in correct order
